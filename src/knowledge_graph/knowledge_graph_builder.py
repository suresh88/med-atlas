...
def build_knowledge_graph(excel_path: str):
    ...
    -------
    graph : nx.DiGraph
        The constructed NetworkX directed graph with nodes and edges.
    stats : dict
    ...
    # Pre‑parse all ICD mappings for the four relationship columns
    relation_cols = {
        'MAY_PREVENT': 'may_prevent_diseases_icd10',
        'CONTRAINDICATED_WITH': 'ci_with_diseases_icd10',
        'MAY_DIAGNOSE': 'may_diagnose_diseases_icd10',
        'MAY_TREAT': 'may_treat_diseases_icd10',
    }

    # Preprocess the ICD mapping for each row per relationship
    parsed_icd: Dict[str, List[Dict[str, List[str]]]] = {rel: [] for rel in relation_cols}
    for rel, col in relation_cols.items():
        for cell in df[col]:
            parsed_icd[rel].append(parse_icd_mapping(cell))
    ...
    for idx, row in df.iterrows():
        ...
        # For each relation type, parse disease→codes mapping
        for rel, col in relation_cols.items():
            mapping = parsed_icd[rel][idx]
            for disease, codes in mapping.items():
        ...
        # Parse FDA approvals and add nodes/edges
        fda_records = parse_fda_data(row.get('fda_approval_data'))
        for record in fda_records:
            # Only include approvals where submission_status == 'AP' (Approved)
            # Some instructions mention submission_type == 'AP', but the dataset
            # uses submission_status to denote approval status.
            if record.get('submission_status') != 'AP':
                continue
            app_no = record.get('application_number')
            submission_type = record.get('submission_type')
            status_date = record.get('submission_status_date')
            if not app_no or not status_date:
                continue
            node_id = f"FDA:{app_no}:{status_date}"
            if node_id not in fda_nodes:
                fda_nodes.add(node_id)
                # Parse date into datetime object if possible
                try:
                    date_obj = datetime.strptime(status_date, '%Y-%m-%d')
                except Exception:
                    date_obj = None
                G.add_node(node_id, type='FDA_Approval', application_number=app_no,
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python knowledge_graph_builder.py <excel_file>")
        sys.exit(1)

    excel_file = sys.argv[1]
    G, stats, report = build_knowledge_graph(excel_file)

    print("Graph statistics:", stats)
    print("Data quality report:", report.to_dict())

    # Export files
    export_graph_to_json(G, "drug_graph.json")
    export_graph_to_graphml(G, "drug_graph.graphml")
    print("Exported drug_graph.json and drug_graph.graphml")    