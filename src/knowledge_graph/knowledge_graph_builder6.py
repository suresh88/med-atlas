...
# ICD10_PATTERN removed as ICD codes are no longer used
...
# _icd_root function removed as ICD codes are no longer used
...
def build_global_graph(df: pd.DataFrame) -> nx.DiGraph:
    ...
    for i, row in df.iterrows():
        ...
        if brand:
            ...
            if brand_node not in seen_brands:
                _add_node(G, brand_node, type="Brand", label=brand)
                seen_brands.add(brand_node)
            _add_edge(G, drug, brand_node, relation="BRANDED_AS")

        # ---- Ingredient nodes
        ingredients = _parse_list_cell(row.get("Ingredient"))
        for ingredient in ingredients:
            ingredient_node = f"Ingredient:{ingredient}"
            if ingredient_node not in seen_ingredients:
                _add_node(G, ingredient_node, type="Ingredient", label=ingredient)
                seen_ingredients.add(ingredient_node)
            _add_edge(G, drug, ingredient_node, relation="CONTAINS")

        # ---- Precise Ingredient nodes
        precise_ingredients = _parse_list_cell(row.get("Precise Ingredient"))
        for precise_ingredient in precise_ingredients:
            precise_ingredient_node = f"PreciseIngredient:{precise_ingredient}"
            if precise_ingredient_node not in seen_precise_ingredients:
                _add_node(G, precise_ingredient_node, type="Precise_Ingredient", label=precise_ingredient)
                seen_precise_ingredients.add(precise_ingredient_node)
            _add_edge(G, drug, precise_ingredient_node, relation="CONTAINS")

        # ---- Diseases from may_treat_diseases
        diseases_list = _parse_list_cell(row.get("may_treat_diseases"))
        ...
        # ---- Diseases from may_treat_diseases_icd10 (only disease names, no ICD codes)
...
def export_pyvis(G: nx.DiGraph, out_html: str):
    """Create an interactive HTML file using PyVis.

    This function attempts to generate a PyVis network and save it to
    ``out_html``.  If PyVis is unavailable or encounters a template
    error (the notorious ``NoneType``/``render`` error), it will
    gracefully skip the interactive export and report a warning.  To
    increase compatibility across environments (including Jupyter
    notebooks), the network is initialised with ``notebook=True`` and
    ``cdn_resources='in_line'`` so that the required HTML template
    resources are embedded directly【848060847654472†L101-L127】.
    ...
    try:
        # Use notebook=True and inline CDN resources to ensure the
        # default template is loaded correctly【848060847654472†L101-L127】.
        net = Network(
            height="850px",
            width="100%",
            directed=True,
        ...
        # Add nodes with type-based colours and labels
        for n, a in G.nodes(data=True):
            t = a.get("type", "Node")
            net.add_node(
                n,
        ...
        # Add edges with relation as tooltip
        for u, v, d in G.edges(data=True):
            net.add_edge(u, v, title=d.get("relation", ""))

        # Define visual options as a JSON string.  PyVis expects valid
        # JSON, so we construct a Python dict and serialise it【848060847654472†L101-L127】.
        options = {
            "edges": {
        ...
        # Write the HTML file.  ``show`` attempts to open the file and
        # display it inline in notebooks, which can trigger template
        # errors.  Instead we call write_html() with notebook=True,
        # ensuring the notebook-aware template is loaded.
        net.write_html(out_html, open_browser=False, notebook=True)
        print(f"Saved interactive HTML: {out_html}")

    except Exception as e:
        # Catch template/render errors and fallback gracefully
        print(f"[WARN] Unable to generate interactive HTML ({e}); skipping.")
...
def main():
    ...
    print("  - drugs_for_disease(G, '<Disease Name>')")
